...
export class BlogService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/blogs");
  ...
  update(blog: IBlog): Observable<EntityResponseType> {
    return this.http.put<IBlog>(
      `${this.resourceUrl}/${getBlogIdentifier(blog) as number}`,
      blog,
      { observe: "response" }
    );
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IBlog[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  addBlogToCollectionIfMissing(
    blogCollection: IBlog[],
    ...blogsToCheck: (IBlog | null | undefined)[]
  ): IBlog[] {
    const blogs: IBlog[] = blogsToCheck.filter(isPresent);
    if (blogs.length > 0) {
      const blogCollectionIdentifiers = blogCollection.map(
        (blogItem) => getBlogIdentifier(blogItem)!
      );
      const blogsToAdd = blogs.filter((blogItem) => {
        const blogIdentifier = getBlogIdentifier(blogItem);
        if (
          blogIdentifier == null ||
          blogCollectionIdentifiers.includes(blogIdentifier)
        ) {
          return false;
        }
